...
- [[#global-todos][Global TODOS]]
  - [[#make-it-so-that-i-dont-have-to-manually-tun-that-code-block-in-firefox-section-in-order-to-tangle-this-file][Make it so that I don't have to manually tun that code block in Firefox section in order to tangle this file]]
...
; Before tangling we need to make =html= code blocks executable by babel so that we can tagnle them
...
#+RESULTS:
| ./firefox-homepage.html | ./doom-emacs/packages.el | ./doom-emacs/config.el | utils.nix | flake.nix | ./.envrc | ./dev-flake/flake.nix |
...
#+begin_quote
Unfortunately what's described in this section doesn't work for whatever reason.
There's a workaround in the next secition that's used instead.
#+end_quote
...
* Global TODOS
** DONE [#B] Make it so that I don't have to manually tun that code block in Firefox section in order to tangle this file
