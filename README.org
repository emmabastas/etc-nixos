...
1) ~config.el~ for configuring thins
2) ~init.el~ for enabling doom-emacs module
3) ~packages.el~ for installing packages that aren't part of any doom-emacs module

~config.el~ and ~packages.el~ are tangled from this document whereas ~init.el~ is edited directly. These three files live in[[file:doom-emacs/][ ./doom-emacs/]]. We tell doom-emacs to find the configuration files there:
...
The configuration in the next block should really be in another, but for now we let it live here:
...
All we need to do is add the following to ~packages.el~
...
** TODO [#C] [[file:~/org/flashcards.org::*Latexpreview sometimes fails][Latexpreview sometimes fails]]
...
** TODO [#B] Configure doom-emacs so that everything doesn't have to be rebuilt everytime
