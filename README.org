...
List current genrations with ~sudo nix-env -p /nix/var/nix/profiles/system --list-generations~
Delete generations older than 30 days, but keep at least 5 generations ~sudo nix-env --delete-generations 30d +5~
Finaly collect the garbage ~sudo nix-collect-garbage~
...
General good guide: https://github.com/james-stoup/emacs-org-mode-tutorial#orga63a848
...
When annotating with noter, we sometimes want to have a headline be a quote, and we add the ~:quoteref:~ keyword, let's customize that particular heading to look like a quote and not a heading. (Actual customization is done in the next block)
