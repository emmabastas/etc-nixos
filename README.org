...
- [[#flashcards-setup][Flashcards setup]]
  - [[#org-drill][=org-drill=]]
  - [[#dynamic-flashcards][Dynamic flashcards]]
...
We define our own flaschard type ~babel_generated~. This card will execute topmost code-block using ~org-babel~ and then display it similar to the ~simple~ card type but without hiding "drawers". Meaning that if you have a flashcard looking something like this:

: *** my test flashcard :drill:
: :PROPERTIES:
: :DRILL_CARD_TYPE: babel_generated
: :END:
...
Add it to the list of flashcard types:
