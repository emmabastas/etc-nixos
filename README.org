...
- [[#misc][Misc]]
  - [[#hours-of-sunlight-wallpaper][Hours-of-sunlight wallpaper]]
    - [[#generating-wallpaper-with-python--matlab][Generating wallpaper with python + matlab]]
      ...
      (system:
        let pkgs = nixpkgs.legacyPackages.${system}; in
        {
          devShells.default = pkgs.mkShell {
            packages = [
              <<hours-of-sunlight-dev-shell-packages>>
            ];
          };
        }
      );
...
* Misc
** Hours-of-sunlight wallpaper
https://nixos.wiki/wiki/I3#Wallpaper

The dark winter days in Stockholm makes me sad, beeing reminded that the theys WILL get longer soon makes it more bearable, so let's display a grph of the hours of daylight for a given day.

*** Generating wallpaper with python + matlab
Almost all code is copied from: https://www.dataliftoff.com/plotting-hours-of-daylight-in-python-with-matplotlib/

The python code depends on ~python~, ~matplotlib~ and ~numpy~

#+name: hours-of-sunlight-deps
#+begin_src nix
...
we need these deps for interactively running the code blocks in this config:

#+name: hours-of-sunlight-dev-shell-packages
#+begin_src nix :noweb yes
<<hours-of-sunlight-deps>>
#+end_src
...
#+begin_src python :results none :session hours-of-daylight-generate :noweb yes :tangle ./hours-of-daylight/generate.py
import math
...
We use[[https://stackoverflow.com/questions/19615350/calculate-sunrise-and-sunset-times-for-a-given-gps-coordinate-within-postgresql][ this StackOverflow answer]]  for calculating the hours of sunligh:

#+begin_src python :results none :session hours-of-daylight-generate :tangle ./hours-of-daylight/generate.py
class Sun:
...
We define a new function for calculating todays hours of daylight given a possition on the globe:
...
We introduce a new function that monkey-patches the ~Sun~ class to give us the daylight hours of any date:
...
We generate a list with the current day and the 182 days preceding and succeeding it:

#+begin_src python :results none :session hours-of-daylight-generate :tangle ./hours-of-daylight/generate.py
dates = [ datetime.datetime.now() + datetime.timedelta(days = t) for t in range(-182, 182)]
#+end_src

Then for each of those dates we associate it's daylight hours (for Stockholm):
...
Now we create a function for plotting data:
...
Finally we write the plot to a file:

#+begin_src python :results file :session hours-of-daylight-generate
outfile = ""
width = 1920/220
height = 1080/220
try:
    try:
        ...
        outfile = os.path.join(tempfile.mkdtemp(), "figure.png")

    print(outfile)

    plt.figure(figsize=(width, height))
    plot(hours_of_daylight_for_dates)
    plt.savefig(outfile)
...
#+RESULTS:
[[file:/tmp/nix-shell.ga6QBh/tmpft_u03_b/figure.png]]
